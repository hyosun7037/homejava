...
public class CookieRun extends JFrame {
	...
	// 장애물 이미지
	ImageIcon attack = new ImageIcon("img/attack.png");
...
//			buffg.drawImage(c, imgX, imgY, null); // 캐릭터
			...
			for (int i = 0; i < attackList.size(); i++) {
				...
				////////////////////// 무적 /////////////////////
				if(tempAttack.getX() < -90) {
					fieldList.remove(tempAttack);
				}else {
					tempAttack.setX(tempAttack.getX() - 4);
					if(
							!invincible
							&& tempAttack.getY() > imgY
							&& tempAttack.getY() < imgY + cookieHeight
							&& tempAttack.getX() > 160
							&& tempAttack.getX() < 240) {
						invincible = true;
...
//			buffg.drawLine(160, 0, 160, 500);
//			buffg.drawLine(240, 0, 240, 500);
